...
srcpath <- "content"
destpath <- "docs"
clean <- FALSE  # set to TRUE to re-render everything
...
tuts_rmd <- dir(srcpath, "\\.[Rr][Mm][Dd]$")
tuts_htm <- dir(destpath, "\\.[Hh][Tt][Mm][Ll]*$")
tuts_out <- tools::file_path_sans_ext(tuts_htm)

if (clean) {
  file.remove(file.path(destpath, tuts_htm))
}
...
for (fname in tuts_rmd) {
  ...
  if (length(destfile) == 1L) {
    ...
    srctime <- file.info(file.path(srcpath, fname))$mtime
    desttime <- file.info(file.path(destpath, destfile))$mtime
  ...
  if (render) {
    ...
    rmarkdown::render(file.path(srcpath, fname),
